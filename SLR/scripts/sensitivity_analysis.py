...
DT_CODES = {
    # Mirrors constitutive_codes in descriptive_themes.py exactly
    "DT1": ["FE-RFLAG","FE-BID","FE-AMEND","FE-BUDGET","CTX-PROCU","CTX-GOVPUB"],
    "DT2": ["MC-IF","MC-LOF","MC-AE","MC-UNSUP","MC-GNN"],
    "DT3": ["CTX-BANK","DS-PRIVATE","DS-TRANS","CTX-AML"],
    "DT4": ["LIM-NOLABEL","AC-LABEL","DS-SYNTH"],
    "DT5": ["CTX-VILLAGE","DS-DANDES","CTX-INDO","DS-GOVERN"],
    "DT6": ["IST-TAM","IST-TTF","IST-DM","IST-IT","IST-AT","IST-DSR","IST-GRC","IST-UTAUT","IST-FRAUD"],
    "DT7": ["GAP-RT","LIM-STATIC","GAP-EXPLAIN"],
    "DT8": ["CTX-DEV","GAP-DC","GAP-VILLAGE","AC-ENGLISH","LIM-SINGLE","LIM-CONTEXT"],
    "DT9": ["MC-GNN","FE-NETW","CTX-SHELL"],
    "DT10":["GAP-EXPLAIN","LIM-INTERP","IST-GRC"],
}
...
report_lines += [
    ...
    "",
    "| DT | Label | T1 (\u22654.0) | T1 % | T2 (\u22654.5) | T2 % | T3 (\u22655.0) | T3 % | Stable? |",
    "|---|---|---|---|---|---|---|---|---|",
]
...
report_lines += [
    ...
    "",
    "| DSR Cycle | T1 (\u22654.0) | T2 (\u22654.5) | T3 (\u22655.0) |",
    "|---|---|---|---|",
]
...
report_lines += [
    ...
    "",
    "**Overall stability**: The core analytical findings — the two-cluster gap structure (AT1),",
    "IS theory absence (AT3), and the DESIGN×village empty cell (F3) — are **robust** to",
    "quality threshold variation. The DESIGN cycle papers remain scarce regardless of threshold.",
    "",
    "**DT5 caveat (important)**: The village fund governance theme (DT5) shows the most",
    "pronounced instability, dropping from 47% of T1 corpus to 17% at T2 and 7% at T3.",
    "This reflects a structural feature of the corpus: village governance papers are",
    "predominantly published in unranked or lower-quartile Indonesian journals, depressing",
    "their composite quality scores despite their high domain relevance. This does NOT",
    "invalidate the DT5 findings — it confirms that domain-override protocol in Phase E",
    "was methodologically necessary. However, claims about DT5 should be presented with",
    "the caveat that they are based primarily on papers in the T1-only (lower quality) tier.",
    "",
    "**Threshold-stable conclusions (robust across T1–T3)**:",
    "- IST-NONE dominates (IS theory absent from detection literature)",
    "- Dana Desa application gap persists (zero DESIGN papers at village level)",
    "- Label scarcity (DT4) remains a structural constraint at all thresholds",
    "- Real-time/explainability deployment gap (DT7, DT10) present across all tiers",
    "",
...
draft_lines += [
    ...
    "proportional representation (variation ≤10 percentage points). The label scarcity theme",
    "(DT4) and the IS theory absence finding remain prominent at all quality levels,",
    "confirming that these findings are not driven by the inclusion of lower-quality papers.",
    "Notably, the village fund governance theme (DT5) shows pronounced instability,",
    "declining from 47% at T1 to 7% at T3; this reflects the structural scoring disadvantage",
    "of domain-relevant papers published in unranked Indonesian journals, and validates the",
    "domain-override protocol applied in Phase E rather than undermining DT5 findings.",
