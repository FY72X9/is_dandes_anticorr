...
**Key Findings:** The Dual-Path Consensus Gate decouples local density isolates (LOF) from multi-model global convergence (IF $\cap$ RDA), isolating **7,153 consensus anomalous activities (7.39%)** representing **Rp 642.85 Miliar in financial realization exposure (14.89% of total provincial disbursement)**. Operational policy mapping reveals a major typology shift toward **T2: Ghost Activities (58.1% of flags)** and **T5: Procurement Irregularities (32.8% of flags)**. Longitudinal persistence tiering identifies **702 villages (51.5%)** flagged across all three fiscal years. On an ex-ante synthetic fraud benchmark, the Dual-Path framework achieves superior recovery (**Precision = 0.846, Recall = 0.846, F1 = 0.846, AUC-ROC = 0.912**).
