...
Two non-exclusive structural hypotheses warrant examination for Phase 2. 

**Structural Hypothesis H1 (Budget Structure)**: Bungo may allocate a disproportionate share of village fund budgets to Swakelola-executed operational activities relative to capital infrastructure — the activity type profile of Bungo's anomaly portfolio (dominated by T1 mark-up and T7 cross-category dump) is consistent with operational budget inflation rather than infrastructure fraud. *This hypothesis is testable: compare Bungo's Kode_Output distribution against the provincial average.*

**Structural Hypothesis H2 (APIP Capacity Gap)**: If Bungo's kabupaten inspectorate operates at reduced staffing or coverage relative to its village count, the probability of detection-induced deterrence is lower, allowing anomalous patterns to persist across years. *This hypothesis is untested in the current study — no APIP staffing data was collected. It should not be treated as a finding but as a candidate explanation for Phase 2 investigation.* The Tier-1 village persistence analysis (103 villages with ≥ 2 flagged years out of 3) is consistent with a deterrence gap, but does not confirm its APIP-capacity mechanism.
...
The across-method anomaly rate profile — 2023 substantially higher than 2024 for all three algorithms (IF: 10.5% → 6.5%, DA: 5.5% → 3.8%, Consensus: 4.1% → 2.0%) — with partial recovery in 2025 (IF: 7.1%, Consensus: 3.3%) forms a distinctive temporal pattern. Two plausible **untested hypotheses** are proposed:

> **Important (v2.0)**: Both hypotheses below require verification against actual Pagu data per year (total village fund allocation in Jambi 2023 vs 2024 vs 2025), which is available from the dataset via F2.4 computation. The hypotheses should not be presented as findings until that data is checked.

**Temporal Hypothesis H3 (Fiscal Expansion)**: The fiscal expansion hypothesis attributes the 2023 spike to budget expansion pressure: Indonesia's central government increased village fund allocations in 2023, expanding the Pagu-realization gap and creating both the opportunity (larger budget to manipulate) and the pressure (mandatory absorption targets) for fraudulent activity. *Testable: check if mean/total Pagu 2023 > 2024 > 2025 in the dataset.*

**Temporal Hypothesis H4 (Compliance Correction)**: The post-correction hypothesis interprets the 2024 trough as genuine administrative improvement — new village fund regulations introduced in 2023 (PP No. 11/2021 implementation guidance) may have produced temporary compliance improvements.
